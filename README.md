...
# 2020.9.28
      字符集编码的理解？
           ：计算机只认识二进制0、1，由0，1演变出ASCII---它是美国专用的，所以为了适应别的语言的需求，进而产生了一系列字符
           集（GBK,GB2312等），中国默认使用的是GBK, 而编程经常使用的utf-8是什么呢，它属于Unicode，Unicode涵盖了全球所有的字
           符，是多个编码方案的统称
# 2020.10.9
      流的分类？
           1.节点流
...
# 2020.10.22
      TCP与UDP在java代码方面的区别？
           ：创建tcp协议实例 ：  new Socket( ip)
                 udp         :  new DatagramSocket()  因为udp不需要连接谁，所以参数也不需要对应ip
                                由于udp发送的都是数据包，所以还需要new DatagramPacket创建数据包
# 2020.10.23
       反射可以在什么时候用呢？
           ：当程序编译阶段时，我们不知道去new 哪个类的对象，只有程序执行时，才能确定，这时就用到反射，利用反射的动态性去创建对应类的实例
             通过反射可以去获取对应类的所有属性及属性对应的权限、类型、变量名
